...
async function sendNotification({to,message,channel='auto'}) {
  const r={};
  if (process.env.TWILIO_ACCOUNT_SID&&process.env.TWILIO_AUTH_TOKEN&&process.env.TWILIO_FROM_NUMBER) {
    try {
      ...
      if (channel==='whatsapp'||channel==='auto') {
        try { await client.messages.create({from:`whatsapp:${process.env.TWILIO_FROM_NUMBER}`,to:`whatsapp:${toNum}`,body:message}); r.whatsapp='sent'; } catch(e){}
      }
      if ((channel==='sms'||channel==='auto')&&!r.whatsapp) { await client.messages.create({from:process.env.TWILIO_FROM_NUMBER,to:toNum,body:message}); r.sms='sent'; }
    } catch(e) { r.twilio_error=e.message; }
  }
  if (process.env.TELEGRAM_BOT_TOKEN&&process.env.TELEGRAM_CHAT_ID) {
    try { await fetch(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chat_id:process.env.TELEGRAM_CHAT_ID,text:message,parse_mode:'HTML'})}); r.telegram='sent'; } catch(e){}
  }
  if (!r.sms&&!r.whatsapp&&!r.telegram) { console.log(`\n📢 [NOTIF→${to}]\n${message}\n`); r.logged=true; }
  return r;
}
...
3. Critical urgency → call trigger_emergency immediately.
...
app.get('/api/health',(req,res)=>res.json({status:'ok',agent:'Aria',system:'ShadowQuant Smart Clinic — Karnataka',districts:Object.keys(KARNATAKA_HOSPITALS).length,total_hospitals:Object.values(KARNATAKA_HOSPITALS).reduce((a,d)=>a+(d.hospitals||[]).length,0),total_clinics:Object.values(KARNATAKA_HOSPITALS).reduce((a,d)=>a+(d.clinics||[]).length,0),model:'gpt-4o-realtime',notifications:{twilio:!!process.env.TWILIO_ACCOUNT_SID,telegram:!!process.env.TELEGRAM_BOT_TOKEN}}));
